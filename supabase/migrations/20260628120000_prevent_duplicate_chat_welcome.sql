-- Prevent duplicate automatic welcome messages when the student area loads twice at the same time.

CREATE UNIQUE INDEX IF NOT EXISTS idx_mensagens_chat_boas_vindas_unica
  ON public.mensagens_chat(conversa_key)
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext(v_personal_id::text), hashtext(v_aluno_id::text));

  SELECT EXISTS (
    SELECT 1
    FROM public.mensagens_chat
    WHERE conversa_key = v_conversa_key
  ) INTO v_existe;
