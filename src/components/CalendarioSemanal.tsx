...
export function CalendarioSemanal({
  ...
  const carregarTreinos = async () => {
    const {
      data,
      error
    } = await supabase.from('treinos_semanais').select('*').eq('profile_id', profileId).eq('semana', format(semanaAtual, 'yyyy-MM-dd'));
    if (error) {
  ...
  const criarTreinosSemana = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('treinos_semanais').insert(novosTreinos).select();
    if (error) {
  ...
  const toggleTreino = async (treinoId: string, concluido: boolean) => {
    const {
      error
    } = await supabase.from('treinos_semanais').update({
      concluido: !concluido
    }).eq('id', treinoId);
    if (error) {
      toast({
    ...
    setTreinos(treinos.map(t => t.id === treinoId ? {
      ...t,
      concluido: !concluido
    } : t));
  };
  return <Card>
      <CardHeader>
        <CardTitle>Frequência de Treinos da Semana</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-7 gap-2">
          ...
          return <div key={index} className={`flex flex-col items-center p-3 rounded-lg border ${isHoje ? 'border-primary bg-primary/5' : 'border-border'}`}>
                ...
                {treino && <Checkbox checked={treino.concluido} onCheckedChange={() => toggleTreino(treino.id, treino.concluido)} />}
              </div>;
        })}
        </div>
      </CardContent>
