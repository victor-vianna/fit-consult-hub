-- Ensures released photo comparisons are available through PostgREST.
-- Safe to run even if the original migration already created the table.
...
CREATE INDEX IF NOT EXISTS idx_foto_comparativos_liberados_personal
  ON public.foto_comparativos_liberados(personal_id, created_at DESC);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.foto_comparativos_liberados TO authenticated;
